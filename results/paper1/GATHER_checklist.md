...
| 7 | Describe methods for calculating uncertainty of the estimates. State which sources of uncertainty were, and were not, accounted for in the uncertainty analysis. | Yes | Methods, Uncertainty quantification (Monte Carlo 1000 draws, SD derivation from GBD UIs); Discussion, Strengths and limitations (limitation ii on model-based estimates) |
...
| 12 | State how the analytic code and data set can be accessed. | Yes | Data sharing statement (GBD Results Tool URL; code repository to be added upon publication) |
