...
\textbf{Findings:} Global age-standardised QCI rose from 91.87 (95\% uncertainty interval [UI] 90.57--93.03) in 1990 to 95.46 (94.68--96.13) in 2021, corresponding to an AAPC of +0.119\% (95\% CI +0.116 to +0.123). Of 195 countries with complete data, 189 improved and six declined between 1990 and 2021. In 2021, QCI ranged from 82.39 (Afghanistan) to 99.94 (Bermuda); 130 countries exceeded a QCI of 95. North America recorded the highest regional QCI (97.82), while Sub-Saharan Africa recorded the lowest (93.11). A clear SDI gradient was observed (Spearman rho = 0.768, p = 2.03 $\times$ 10\textsuperscript{--31}), with low-SDI settings converging faster (AAPC +0.135\%) than high-SDI settings (+0.093\%). Females had higher QCI than males globally (96.13 vs 94.88), and children aged 5--14 years had the highest age-specific QCI (97.82) while those under 5 years had the lowest (92.61). In a striking departure from the global trend, six countries experienced net QCI declines over the study period --- Zimbabwe (--7.82 points), Lesotho (--6.61), Eswatini (--3.00), Kenya (--0.99), Libya (--0.55), and Burkina Faso (--0.10) --- with the four sub-Saharan declines concentrated in the period of peak HIV/TB syndemic burden (broadly 1995--2008) before partial recovery after antiretroviral scale-up. These reversals are the most policy-actionable finding of the analysis and concentrate disproportionately in southern Africa.

\textbf{Interpretation:} Substantial but uneven progress in DS-TB care quality has occurred over three decades. Six countries reversed prior gains during the HIV/TB syndemic, and Sub-Saharan Africa, low-SDI settings, males, and children under 5 years continue to bear disproportionate residual burden. These reversal trajectories --- and the partial recoveries that followed antiretroviral scale-up in some of the same countries --- highlight TB care quality as a fragile equilibrium that can be lost as well as gained. The QCI offers a reproducible, composite benchmark for monitoring TB care quality at the population level and can inform resource allocation toward the 2030 End TB Strategy targets.
...
We assessed construct validity against three external benchmarks of progressively narrower domain. First, Spearman rank correlation between country-level QCI in 2021 and the GBD Socio-demographic Index (SDI), a composite measure of per-capita income, educational attainment, and fertility rate.\textsuperscript{8,32} SDI captures the broad socioeconomic determinants that underpin health system capacity but is not itself a healthcare indicator. Second, the GBD Healthcare Access and Quality (HAQ) Index, a domain-relevant healthcare-system performance index that synthesises mortality-to-incidence ratios across 32 amenable causes,\textsuperscript{6} compared at year 2019 (the latest year present in our locally-archived HAQ release). Third, the WHO Treatment Success Rate for newly notified DS-TB cases under post-2012 outcome definitions (variable c\_new\_tsr from the WHO Global TB Programme Treatment Outcomes bulk download, accessed 9 May 2026), compared at year 2019. The three validators differ qualitatively: SDI is upstream of healthcare; HAQ is a broad cross-condition healthcare composite; WHO TSR is a programme-level treatment-pipeline indicator. Concordance with SDI and HAQ supports general construct validity; concordance --- or lack thereof --- with WHO TSR helps locate the QCI within the existing TB-monitoring landscape.
...
Country-level validation against SDI confirmed a strong monotonic association (Spearman rho = 0.768, p = 2.03 $\times$ 10\textsuperscript{--31}; Pearson r = 0.712, p = 2.71 $\times$ 10\textsuperscript{--25}; n = 155 countries). Validation against the GBD HAQ Index at year 2019 produced a stronger correlation (Spearman rho = 0.824, p = 2.03 $\times$ 10\textsuperscript{--49}; Pearson r = 0.738, p = 8.94 $\times$ 10\textsuperscript{--35}; n = 195 countries), providing direct convergent evidence with an established healthcare-system performance index. By contrast, validation against the WHO Treatment Success Rate (c\_new\_tsr) at year 2019 yielded a negative correlation (Spearman rho = --0.350, p = 1.60 $\times$ 10\textsuperscript{--6}; n = 179 countries). This negative association is interpretable rather than anomalous: WHO TSR is computed as a ratio over notified cases only, so countries with weak TB-notification systems can simultaneously record high TSR (the small number of cases entering the treatment pipeline are managed effectively) and low QCI (substantial unnotified mortality lowers the index via GBD's incidence imputation). The QCI thus complements rather than replaces WHO TSR by capturing population-level outcomes that the notification-conditional TSR cannot directly observe. Importantly, low-SDI countries exhibited faster QCI convergence (AAPC +0.135\%, +0.130 to +0.141) than high-SDI countries (+0.093\%, +0.083 to +0.103), suggesting a narrowing of the development-quality gap over time. All 45 high-SDI countries (100\%) achieved QCI above 90 in 2021, compared with 33 of 40 low-SDI countries (82.5\%).
...
The WHO treatment success rate, the most widely used TB programme indicator, measures only the end of the treatment cascade and does not account for undiagnosed cases, diagnostic delays, or post-treatment sequelae.\textsuperscript{1} The QCI, by contrast, captures the net outcome of the entire care pathway from disease onset to resolution or death, albeit at the ecological rather than individual level. This conceptual difference is empirically visible: across 179 countries with both indicators in 2019, QCI and WHO TSR were negatively correlated (Spearman $\rho$ = --0.35; Table~\ref{tab:table4}). The most plausible explanation is that TSR is conditioned on notification --- it counts only patients who entered the treatment pipeline --- while the QCI is constructed from population-level mortality and disease-burden ratios that are also sensitive to unnotified cases. In settings with weak case-finding, the small fraction of notified patients can be managed effectively (yielding a high TSR) while the bulk of incident cases die or remain disabled untreated (lowering the QCI). The two metrics are therefore complementary rather than redundant: TSR remains the appropriate indicator for treatment-pipeline performance, while the QCI summarises the outcome of the entire population-level care cascade. Reassuringly, against the GBD Healthcare Access and Quality Index --- which, like the QCI, is built from outcome ratios rather than notifications --- the QCI shows strong positive concordance (Spearman $\rho$ = 0.82, n = 195; Table~\ref{tab:table4}).
...
The six countries with net QCI decline present a distinctive epidemiological profile. Zimbabwe (--7.82 points) and Lesotho (--6.61 points) experienced catastrophic QCI declines that the well-established HIV/TB syndemic\textsuperscript{14} closely tracks: HIV increases TB incidence, complicates diagnosis, reduces treatment effectiveness, and amplifies mortality. The simple full-period AAPCs we report (Zimbabwe --0.357\%/yr, Lesotho --0.263\%/yr) average across distinct epidemiological eras, so we additionally fitted a piecewise log-linear (joinpoint) regression with up to three segments selected by Bayesian Information Criterion. For Zimbabwe, the BIC-preferred fit identified joinpoints at approximately 1998 and 2008 and segment AAPCs of --0.20\%/yr (1990--1998), --0.81\%/yr (1998--2008, the catastrophic HIV-driven phase), and +0.087\%/yr (2008--2021, post-antiretroviral recovery). Lesotho's segmentation followed a similar shape with breaks around 2002 and 2005. Eswatini --- whose full-period AAPC of --0.07\%/yr suggests near-stagnation --- in fact reversed sharply from --0.81\%/yr before 1995 to +0.46\%/yr after 2004; the 1-segment specification masks this recovery. Kenya showed a smaller but qualitatively similar pattern. Libya and Burkina Faso showed marginal declines that may reflect data quality fluctuations rather than true deterioration in care quality. The joinpoint findings reinforce the headline observation that DS-TB care quality is a fragile equilibrium that can be lost as well as gained, and that countries currently lower in the QCI ranking include both ones still in decline and ones that have already begun to recover.
...
\begin{table}[htbp]
\centering
\caption{Construct validation of the QCI against three external benchmarks. Cross-country Spearman and Pearson correlations between QCI and: (i) the GBD Socio-demographic Index (SDI) at 2021, (ii) the GBD Healthcare Access and Quality (HAQ) Index at 2019, and (iii) the WHO Treatment Success Rate (c\_new\_tsr) at 2019.}
\label{tab:table4}
\small
\begin{tabular}{lccccl}
\toprule
External validator & Year & Spearman $\rho$ & Pearson $r$ & N & Interpretation \\
\midrule
SDI (GBD 2021) & 2021 & 0.768 & 0.712 & 155 & Strong, expected \\
HAQ Index (GBD 2019)\textsuperscript{6} & 2019 & 0.824 & 0.738 & 195 & Stronger; convergent \\
WHO TSR (c\_new\_tsr) & 2019 & --0.350 & --0.290 & 179 & Negative; complementary \\
\midrule
\multicolumn{6}{l}{\textit{Distributional summary of QCI (2021)}} \\
\midrule
Mean (SD) & 2021 & \multicolumn{2}{c}{95.68 (2.83)} & 195 & -- \\
Median (IQR) & 2021 & \multicolumn{2}{c}{96.38 (3.47)} & 195 & -- \\
Range & 2021 & \multicolumn{2}{c}{82.39--99.94} & 195 & -- \\
\bottomrule
\end{tabular}

\vspace{0.5em}
\noindent\small All correlations significant at $p < 0.001$. The negative QCI--WHO TSR correlation is interpretable rather than anomalous (see text): TSR is conditioned on notification, while QCI captures outcomes among notified and unnotified cases combined.
\end{table}
