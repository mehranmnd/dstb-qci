...
| 7 | Describe methods for calculating uncertainty of the estimates. State which sources of uncertainty were, and were not, accounted for in the uncertainty analysis. | Partially | Methods, Concentration Index (jackknife SE, 95% CI); Methods, SII (OLS SE); Discussion, Limitations vi (GBD uncertainty not propagated through inequality calculations) and viii (Wagstaff $R^2=1$ as structural rather than causal). Sources of uncertainty NOT accounted for: (a) GBD-level uncertainty in the underlying QCI inputs is not propagated into the CI, SII, Gini, Theil, Wagstaff, or convergence estimates; (b) the multilevel regression standard errors assume the QCI is observed without error, which overstates precision; (c) the convergence half-life point estimate is not accompanied by a bootstrap interval. |
