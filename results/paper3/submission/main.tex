...
\textbf{Findings:} Iran's national age-standardised QCI rose from 93.97 (95\% UI 92.58--95.10) in 1990 to 97.42 (96.86--97.86) in 2021, an AAPC of +0.097\% (95\% CI +0.084 to +0.109). All 31 provinces improved and all exceeded a QCI of 95 by 2021. Chahar Mahaal and Bakhtiari had the highest QCI (98.77), while Sistan and Baluchistan had the lowest (95.20), yielding a provincial range of 3.58 points in 2021 compared with 7.48 points in 1990. The CV halved from 1.744\% to 0.710\%, and the Gini coefficient declined from 0.0094 to 0.0036, demonstrating strong sigma-convergence. Beta-convergence was confirmed ($r = -0.958$, $p < 0.001$), with provinces that had lower 1990 QCI improving faster. Sistan and Baluchistan showed the largest absolute gain (+5.36 points) despite remaining the lowest-ranked province. Females had consistently higher QCI than males across all provinces (mean GDR 1.012), with the gap largest in Lorestan (GDR 1.021) and smallest in Alborz (1.004). Iran's national QCI of 97.42 ranked fifth among 19 Middle East and North Africa countries in 2021, above the regional average.
...
\textbf{Interpretation:} Iran has achieved high and progressively more equitable DS-TB care quality across all 31 provinces, with pronounced convergence over three decades. Residual gaps in Sistan and Baluchistan, Golestan, and southeastern provinces are accounted for by a combination of excess disease burden and elevated case fatality, with the YLL-to-YLD ratio contributing the largest share of the QCI gap in the structural decomposition. Targeted strengthening of TB diagnostic and treatment services in these regions, informed by province-specific component profiles, could further narrow subnational disparities.
...
Global Moran's I confirmed statistically significant positive spatial autocorrelation in provincial QCI at all four time points assessed (Table~\ref{tab:table2}; Figure~\ref{fig:figure11}). In 1990, Moran's I was 0.324 ($p = 0.002$), indicating moderate clustering of similar QCI values among geographically neighbouring provinces. By 2021, Moran's I had declined to 0.211 ($p = 0.022$), indicating persistent but attenuated spatial clustering. Results were robust to the choice of spatial weights (KNN-5 sensitivity: $I = 0.216$, $p = 0.008$ for 2021). The decline in Moran's I over time paralleled the convergence trend, reflecting the progressive homogenisation of provincial QCI. To contextualise the magnitude of the observed I values relative to chance, we constructed the empirical null distribution from 999 spatial permutations of provincial QCI for each year. The null distribution is centred near $E[I] = -1/(n-1) \approx -0.033$ with a 95\% reference interval of approximately [--0.21, +0.20]. Observed I values fall consistently above the upper null bound: 1990 I = 0.324 lies at the 99.6th percentile of its null distribution, 2010 I = 0.216 at the 99.1st, and 2021 I = 0.211 at the 97.8th (Supplementary Figure~S2). Spatial clustering is therefore not consistent with random spatial arrangement and is unlikely to reflect chance covariance alone.
...
Shapley decomposition apportioned the QCI gap from the best-performing province across all three component ratios, with the YLL-to-YLD ratio contributing the largest share on average (37.2\%), followed by the DALY-to-prevalence ratio (34.9\%) and the MIR (27.8\%) (Table~\ref{tab:table6}; Figure~\ref{fig:figure13}). Because the QCI is a linear function of the three ratios by construction, this attribution is a structural decomposition of the index's own variance and does not by itself establish independent causal contributions. The choice of reference province is a sensitivity question: replacing the single best province with the mean of the top-five provinces' component ratios as a more robust reference produces nearly identical attribution shares for the lagging provinces with material gaps. For example, Sistan and Baluchistan (the largest gap) attributes 27.9\% of its gap to MIR under the single-best reference and 28.6\% under the top-5-mean reference; YLL/YLD shares 36.6\% versus 36.2\%; DAL/PER shares 35.5\% versus 35.2\%. Per-province attributions for the lower-performing provinces are therefore robust to the reference choice; the all-31-province mean is more sensitive (because the top-five themselves register near-zero gaps under their own group mean) and we accordingly emphasise per-province profiles over the all-province mean in the policy discussion below.
...
West Azarbayejan showed the highest MIR contribution (36.6\%) among all provinces, indicating that case fatality rather than disease burden accounts for the largest share of its structural QCI gap.
...
The Shapley decomposition provides a structural map of where provincial QCI gaps sit, which can guide hypothesis generation for targeted interventions; it does not, on its own, identify causal determinants.\textsuperscript{21} The finding that the YLL-to-YLD ratio contributes the largest share (37.2\%) of the average provincial QCI gap indicates that the premature-mortality-versus-disability balance accounts for the largest single fraction of inter-provincial variation in the index. This ratio captures the balance between death and morbidity --- provinces with high YLL-to-YLD ratios have a disproportionate burden of premature TB deaths relative to the disability experienced by surviving patients.
...
A further methodological caveat is specific to subnational analyses such as ours. Provincial-level GBD estimates for Iran are not direct measurements but model outputs that rely heavily on covariate borrowing across provinces and over time, particularly in provinces with sparse vital registration or surveillance.\textsuperscript{15,16} This has two consequences for our spatial findings. First, the positive Global Moran's I values we report (0.211--0.324) and the LISA clusters we identify partly reflect the spatial structure that GBD imposes through its covariate-based smoothing, not only genuinely heterogeneous patterns of care delivery; we therefore interpret the spatial patterns as ``consistent with'' rather than ``demonstrative of'' real geographic clustering. Second, the decline in Moran's I over time may partly reflect improving GBD data inputs (more national surveillance and vital-registration data over the 1990--2021 period) rather than only real provincial homogenisation. Triangulation against direct provincial surveillance data --- where available from Iran's National TB Programme --- would be needed to disentangle these effects, and we consider this a high-priority direction for future research. The Shapley decomposition of provincial QCI gaps, which by construction attains $R^2 = 1.000$ because the QCI is a linear function of its component ratios, should likewise be read as a structural attribution of the index's own variance rather than as a causal identification of independent provincial drivers; province-specific component profiles remain useful as descriptive guides to where the largest sub-components of the gap sit, but causal interpretation of the percentage shares is not warranted from the present data.
